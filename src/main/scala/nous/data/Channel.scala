...
class Channel[A: ClassTag](h: Int, w: Int, elems: Vector[A]) { self =>
  val data = elems
  val height = h
  val width = w
  ...
  def update(e: Vector[A]): Channel[A] = {
    require(e.length == height * width, "Updated data length is inconsistent with previous dimensions, use updated(h, w, e) instead")
    update(h, w, e)
  }

  def update(h: Int, w: Int, e: Vector[A]): Channel[A] = new Channel(h, w, e)

}

object Channel {
  def apply[A: ClassTag](h: Int, w: Int, data: Vector[A]): Channel[A] = new Channel(h, w, data)
}