...
object convolution {
  ...
  def pool[A](
    ...
    Channel(pooledH, pooledW, outArray)
  }
   ...
   */
  def conv1[A: ClassTag](input: Array[A], kernel: Array[A])(implicit ev: Numeric[A]) = {
    val hs = kernel
    val xs = input
  ...
  def very_naive_conv2[A: ClassTag](input: Array[A], kernel: Array[A], padding: Int, stride: Int)(implicit ev: Numeric[A]): Stream[Pure, A] = {
    // Static sizes
