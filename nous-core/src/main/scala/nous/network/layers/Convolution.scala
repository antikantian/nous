...
class Convolution[A: ClassTag](cdef: ConvDefinition[A], in: InputShape, w: Array[A], id: Int = -1)(
  ...
  val bias =
    if (definition.bias)
      Array.fill(definition.filters)(field.zero)
    else
      Array.empty[A]
  ...
  def forward(x: Sample[A, A]): Sample[A, A] = {
    ...
    if (bias.nonEmpty) {
      ...
      x.update(outputC, outputH, outputW, yb)
    } else {
      x.update(outputC, outputH, outputW, y)
    }
  }

  def backward(y: Array[A], gradient: Array[A]): Array[A] = {
    W.data
  }

  def updateW(weights: Array[A]): Convolution[A] = {
    self
  }
