...
import spire.math.{ConvertableTo, Numeric, Rational}
...
class ConvolutionKernelTest extends FunSuite {
  ...
  test("convolution 1d with float") {
    val time = measure {
      val input = Array[Float](10, 50, 60, 10, 20, 40, 30)
      val kernel = Array[Float](0.3f, 0.3f, 0.3f)
      conv1(input, kernel)
    }
  ...
  test("convolution 1d with double") {
    val time = measure {
      val input = Array[Double](10, 50, 60, 10, 20, 40, 30)
      val kernel = Array[Double](0.3d, 0.3d, 0.3d)
      conv1(input, kernel)
    }
...
object ConvolutionKernelTest {
  ...
  def makeRandom[A: ClassTag](rows: Int, cols: Int, channels: Int)(implicit ev: Numeric[A], ev1: ConvertableTo[A]) = {
    val lower = -5.0
    val upper = 5.0
