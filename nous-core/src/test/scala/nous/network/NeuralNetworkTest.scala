...
class NeuralNetworkTest extends FunSuite {
  ...
  test("neural network cycle") {
    val layerdef1 = FcDefinition[Float](128, bias = false, init, Sigmoid[Float]())
    //val layer1do = DropoutDefinition[Float](0.2)
    val layerdef2 = FcDefinition[Float](128, bias = false, init, Sigmoid[Float]())
    //val layer2do = DropoutDefinition[Float](0.2)
    val layerdef3 = FcDefinition[Float](1, bias = false, init, Sigmoid[Float]())

    val net = NeuralNetwork.build[Float](List(layerdef1, layerdef2, layerdef3), testInput.shape, lossFunction)
    //val forward = net.prop(testInput)
    //val backward = net.backpropagate(forward).value

    //println(s"${backward.value.mkString(", ")}")

    val output = net.train1(testInput.head)
